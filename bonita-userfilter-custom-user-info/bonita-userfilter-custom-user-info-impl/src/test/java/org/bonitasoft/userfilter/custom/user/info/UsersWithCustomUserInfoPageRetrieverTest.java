...
public class UsersWithCustomUserInfoPageRetrieverTest {
    ...
    public void setUp() throws Exception {
        retriever = new UsersWithCustomUserInfoPageRetriever(identityAPI, CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, MAX_PAGE_SIZE );
    }
    ...
    public void nextPage_should_retrive_the_next_available_page_and_move_pointer_to_the_following_one() throws Exception {
        //given
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, 0, MAX_PAGE_SIZE)).thenReturn(Arrays.asList(1L, 2L));
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, MAX_PAGE_SIZE, MAX_PAGE_SIZE)).thenReturn(Arrays.asList(3L, 4L));
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, 2 * MAX_PAGE_SIZE, MAX_PAGE_SIZE)).thenReturn(Arrays.asList(5L));
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, 3 * MAX_PAGE_SIZE, MAX_PAGE_SIZE)).thenReturn(Collections.<Long>emptyList());
